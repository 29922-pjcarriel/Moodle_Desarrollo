...
export function StudentCourses({ onSelectCourse, onOpenMessages }: StudentCoursesProps) {
  const handleEnterCourse = (courseTitle: string) => {
    onSelectCourse(courseTitle);
  };

  const handleMessageTeacher = (courseTitle: string) => {
    onOpenMessages(courseTitle);
  };
    ...
    <div className="max-w-7xl mx-auto px-6 py-8">
      <div className="mb-8">
        ...
        <p className="text-[#666666]">
          Selecciona un curso para ver su contenido y actividades.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {courses.map((course) => (
          <CourseCard
            ...
            onMessage={() => handleMessageTeacher(course.title)}
          />
        ))}
      </div>
    </div>
